...
function mountScrollWorld(container, config) {
  ...
  // PATCH (Alhadi Cyber Media): `reduce` tetap mematikan SEMUA gerak dekoratif —
  // partikel, ken-burns pada poster, parallax teks, easing smooth-scroll, dan
  // lerp rAF (di bawah reduce lerp = 1, jadi frame video mengikuti posisi scroll
  // persis 1:1, tanpa gerak sisa). `videoUnderReducedMotion: true` hanya
  // mengizinkan klip yang di-scrub tetap dimuat, karena di halaman ini klip
  // ITULAH kontennya dan geraknya sepenuhnya dikendalikan scroll pengunjung —
  // tidak ada animasi yang berjalan sendiri. Default tetap perilaku repo asli.
  const reduceBlocksVideo = reduce && !config.videoUnderReducedMotion;
  ...
  function loadClip(s) {
    ...
    // (Kecuali config.videoUnderReducedMotion = true — lihat catatan PATCH di atas.)
    if (reduceBlocksVideo || s.loading || !s.clip) return;
    s.loading = true;
